use crate::util::{send, RawRequest};
...
use super::Case;
use crate::util::raw_http::RawRequest;
...
pub fn run(base: &str) {
    ...
    println!("running {} compliance cases\n", CASES.len());

    for case in CASES {
        let (status, connection_closed) = send(base, &case.req);
        ...
        if status != case.expect_status {
            ...
            eprintln!(
                ...
                case.name, case.expect_status, status
            );
        }

        if case.expect_close & &!connection_closed {
            ok = false;
            eprintln!("error: {}: expected connection close", case.name);
        }
    ...
    println!();
    println!("compliance result: {} passed, {} failed", passed, failed);

    if failed > 0 {
        eprintln!("warning: compliance failures detected");
    }
}
